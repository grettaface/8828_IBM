...
gulp.task('styles', () => {
  return (
    gulp
      .src(['src/vendor/**/*.css', 'src/sass/*.scss'])
...
gulp.task('static', () => {
  return gulp
    .src(
      'src/**/*.{html,php,jpg,jpeg,png,gif,webp,mp4,svg,ico,eot,ttf,woff,woff2,otf,mp3,wav}'
    )
...
gulp.task('serve', ['styles', 'scripts', 'templates', 'static'], () => {
  ...
  gulp
    .watch(
      [
        'src/**/*.{html,php,jpg,jpeg,png,gif,webp,mp4,svg,ico,eot,ttf,woff,woff2,otf}'
      ],
      ['static']
    )
    .on('change', event => {
